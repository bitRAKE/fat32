...
static int fault_read(void *v,uint64_t lba,void *out) { return sb_read(&((Fixture *)v)->buffer,lba,out); }
...
static void test_basic(unsigned bytes,unsigned spc) {
    ...
    OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("Long file name Ω.bin")); CHECK(e.size==70000);
...
static void test_geometry(void) {
    Fixture *f=fixture(512,128); unsigned char *p=page(&f->disk,0,0)->data;
    report("BPB rejection / truncated extent / reserved and cyclic FAT entries");
    p[13]=3; CHECK(fat_mount(&f->id,&f->fault,NULL)==F_FORMAT); p[13]=128;
    wr16(p+11,1024); CHECK(fat_mount(&f->id,&f->fault,NULL)==F_FORMAT); wr16(p+11,512);
    wr16(p+42,1); CHECK(fat_mount(&f->id,&f->fault,NULL)==F_FORMAT); wr16(p+42,0);
    f->fault.sectors--; CHECK(fat_mount(&f->id,&f->fault,NULL)==F_FORMAT); f->fault.sectors++;
    fat_value(&f->disk,0,2,2); CHECK(fat_mount(&f->id,&f->fault,NULL)==F_CORRUPT);
    fat_value(&f->disk,0,2,0x0FFFFFF7); CHECK(fat_mount(&f->id,&f->fault,NULL)==F_CORRUPT);
    fat_value(&f->disk,0,2,0); CHECK(fat_mount(&f->id,&f->fault,NULL)==F_CORRUPT);
    fat_value(&f->disk,0,2,0x0FFFFFFF); OK(fat_mount(&f->id,&f->fault,NULL)); destroy(f);
}
...
static void test_active_fat(void) {
    ...
    memcpy(original,page(d,32,0)->data,512); OK(fat_mount(&f->id,&f->fault,NULL));
...
static void test_corruption(void) {
    ...
    report("malformed directory clusters / mismatched mirrored FAT write rejection");
    p=page(&f->disk,f->disk.data,1)->data;
    ...
    CHECK(fat_write(&f->id,&e,&t)==F_CORRUPT); CHECK(e.cluster==0); destroy(f);
}
...
static void test_lfn_damage(void) {
    ...
    p[13]^=1; OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("F0000001")); CHECK(e.lfn_count==0);
    memcpy(p,raw,512); p[0]=0x42; OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("F0000001")); CHECK(e.lfn_count==0);
    memcpy(p,raw,512); wr16(p+1,0xD800); OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("F0000001")); CHECK(e.lfn_count==0);
    memcpy(p,raw,512); wr16(p+24,'x'); OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("F0000001")); CHECK(e.lfn_count==0);
    memcpy(p,raw,512); OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("long.txt")); CHECK(e.lfn_count==1);
    destroy(f);
}
...
static void test_media_errors(void) {
    ...
    OK(sb_commit(&f->buffer)); OK(fat_mount(&f->id,&f->fault,NULL)); e=lookup(f,2,U("failure"));
